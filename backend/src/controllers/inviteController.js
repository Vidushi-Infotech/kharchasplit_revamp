...
const createInvite = async (req, res, next) => {
  try {
    ...
    // Generate invite codes and build batch INSERT
    const inviteCodes = phoneNumbers.map(() => generateInviteCode());
    const values = [];
    const params = [];
    let paramIndex = 1;
    ...
    for (let i = 0; i < phoneNumbers.length; i++) {
      ...
      params.push(inviteCodes[i], userId, phoneNumbers[i], contextJson, expiresAt, 'pending');
      paramIndex += 6;
    }

    // Single batch INSERT for all invites
    ...
    // Build response from batch result
    const invites = result.rows.map(invite => {
      const shareUrl = `https://kharchasplit.com/invite/${invite.invite_code}`;
      const shareMessage = context?.groupName
        ? `Join me on KharchaSplit! I've added you to "${context.groupName}". Use my invite code: ${invite.invite_code}\n\nDownload: ${shareUrl}`
        : `Join me on KharchaSplit! Use my invite code: ${invite.invite_code}\n\nDownload: ${shareUrl}`;
      return {
